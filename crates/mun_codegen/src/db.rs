...
#[derive(Debug)]
pub struct IrDatabase<D: hir::HirDatabase> {
...
impl<D: hir::HirDatabase> IrDatabase<D> {
    pub fn new(hir_db: D) -> IrDatabase<D> {
    ...
    pub fn struct_ty<'ink>(&self, context: &'ink Context, s: hir::Struct) -> StructType<'ink> {
        crate::ir::ty::struct_ty_query(context, self, s)
    }
    ...
    pub fn type_info<'ink>(&self, context: &'ink Context, ty: hir::Ty) -> TypeInfo {
        crate::ir::ty::type_info_query(context, &self.target_data, self, ty)
    }
}
